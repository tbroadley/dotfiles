...
if _agent_allowed codex; then
  add_to_rc "codex()" 'codex() { _check_agent_allowed codex && command codex --dangerously-bypass-approvals-and-sandbox --add-dir /opt/python --add-dir "$HOME/.cache/uv" "$@"; }'
  add_to_rc "cx()" 'cx() { _check_agent_allowed codex && command codex --dangerously-bypass-approvals-and-sandbox --add-dir /opt/python --add-dir "$HOME/.cache/uv" "$@"; }'
  add_to_rc "cxr()" 'cxr() { _check_agent_allowed codex && command codex resume --dangerously-bypass-approvals-and-sandbox --add-dir /opt/python --add-dir "$HOME/.cache/uv" "$@"; }'
else
...
if _agent_allowed pi; then
  ...
  if [ -d "$SCRIPT_DIR/pi/agent" ]; then
    ...
    _pi_dotfiles_settings="$SCRIPT_DIR/pi/agent/settings.json"
    _pi_local_settings="$PI_AGENT_DIR/settings.json"
    if [ -f "$_pi_local_settings" ] && [ ! -L "$_pi_local_settings" ] && command -v jq >/dev/null 2>&1; then
      # Local file exists and is not a symlink: merge dotfiles on top of local
      # dotfiles keys win, but local-only keys are preserved
      jq -s '.[0] * .[1]' "$_pi_local_settings" "$_pi_dotfiles_settings" > "$_pi_local_settings.tmp" \
        && mv "$_pi_local_settings.tmp" "$_pi_local_settings"
    else
      # No local file or it's a stale symlink: copy from dotfiles
      rm -f "$_pi_local_settings"
